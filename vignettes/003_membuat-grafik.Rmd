...
gdp_id <- 
  ...
  rename_all(~str_remove_all(., "(?:gdp_on_|_sector_in_idr_million_constant_price|_in_idr_million_constant_price)")) %>% # Untuk Me-rename dan menghapus kata didalam "str_remove_all"
...
gdp_id %>% 
  ...
  ggplot(aes(x=manufacturing,y=trade_hotel_and_restaurant, colour = province )) +
  geom_point(alpha = 0.5) +  #fungsi Alpha adalah untuk transparan
  scale_size_area()
```
...
ggplot(jabar_road, aes(x = district, y = road_length,fill=condition)) +
  ...
  theme(axis.text = element_text(angle=90))#untuk mengatur letak tulisan
```
...
ggplot(jabar_road, aes(x = district, y = road_length,fill=condition)) +
  geom_col(position = "dodge")+
  theme (
    axis.text = element_text(angle = 90) # buat miringin teks yang dibawah 
  )
```
